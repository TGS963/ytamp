//! Draws ytamp's built-in Winamp skin: the one the skinned window wears
//! until the listener picks another, and the one any skin's missing
...
//! examples/default_skin.rs.
//!
//! `cargo run --example default_skin` regenerates
//! `assets/skins/builtin.wsz`, a classic skin in every respect (BMPs in
//! a zip) that Winamp itself could wear. Pass a path to write somewhere
//! else. Add `--preview <png>` to also compose the main window at 2x
//! through the same sprite and layout tables the app uses, as a check
//! that the art lines up.

use std::path::PathBuf;

use ytamp::skin::layout::{self, Area};
use ytamp::skin::sprites::{self, Sheet, Sprite};
use ytamp::skin::{Skin, font, zip};

type Rgb = [u8; 3];
/// A box on a canvas: x, y, width, height.
type Box = (i64, i64, i64, i64);

// ytamp's dark palette (src/theme/default.rs), so the window matches the
// app. Each constant names the theme role it comes from; a few extra
// shades round out button and outline states the theme has no name for,
// staying dark and neutral like the rest of the palette.
const WINDOW: Rgb = [18, 18, 20]; // theme PageBackground
const PANEL: Rgb = [26, 26, 30]; // theme PanelBackground
const SURFACE: Rgb = [30, 30, 35]; // between PANEL and SURFACE_HOVER, a resting button
const SURFACE_HOVER: Rgb = [38, 38, 44]; // theme RowHover
const SURFACE_ACTIVE: Rgb = [48, 48, 55]; // brighter than SURFACE_HOVER, a pressed button
const OUTLINE: Rgb = [42, 42, 48]; // a border between PANEL and SURFACE_HOVER
const TEXT: Rgb = [235, 235, 235]; // theme TextPrimary
const SECONDARY: Rgb = [160, 160, 165]; // theme TextSecondary
const DIM: Rgb = [90, 90, 95]; // a label dimmer than SECONDARY, brighter than PANEL
const ACCENT: Rgb = [255, 64, 64]; // theme Accent
const ON_ACCENT: Rgb = WINDOW; // dark enough to read on top of ACCENT
/// Lamps that are off: the accent, darkened almost to the background.
const ACCENT_DARK: Rgb = [115, 30, 30];
/// The visualiser's peak marks: theme Danger, a warmer red than the accent.
const PEAK: Rgb = [255, 110, 90];
...
impl Canvas {
    ...
    /// The app's mark: a rounded square with a play triangle. `radius` is
    /// the half-width of the square, so the mark is `2 * radius + 1` pixels
    /// on a side; the corners are chamfered in proportion to `radius`, so
    /// the mark still reads as rounded at small sizes.
    fn logo(&mut self, cx: i64, cy: i64, radius: i64, mark: Rgb, glyph: Rgb) {
        let size = 2 * radius + 1;
        let chamfer = (radius / 3).max(1);
        for dy in 0..size {
            for dx in 0..size {
                let from_left = dx.min(size - 1 - dx);
                let from_top = dy.min(size - 1 - dy);
                if from_left + from_top < chamfer {
                    continue;
                }
                self.set(cx - radius + dx, cy - radius + dy, mark);
            }
        }
        let half = radius / 2;
        for dy in -half..=half {
...
fn title_bar(c: &mut Canvas, x: i64, y: i64, active: bool, shade: bool) {
    ...
    if shade {
        ...
        c.text_centred((x, y + 1, 275, 12), "YTAMP", color);
        let width = 5 * 5 - 1;
...
fn pledit_sheet() -> Canvas {
    ...
    // The left half's menus, painted and quiet: ytamp has no files to
    // add, but a bottom without them looks unfinished.
...
fn preview(skin: &Skin) -> image::RgbaImage {
    ...
    text("YTAMP - A WINAMP SKIN *** ", layout::MARQUEE);
...
/// The sheets, in the order they are packed. Kept as a function so `main`
/// stays a plain read-write-report sequence.
fn sheets() -> [(Sheet, Canvas); 15] {
    [
...
/// Packs the sheets and the two text files into a `.wsz` archive.
fn archive() -> Vec<u8> {
    let sheets = sheets();
...
/// Writes the preview PNG the `--preview` flag asked for, composed from
/// the archive just written, at 2x through the real sprite tables.
fn write_preview(archive: &[u8], path: &std::path::Path) {
    let skin = Skin::from_archive("ytamp", archive).expect("the skin just written reads");
...
/// The two paths `main` can take on the command line: where to write the
/// skin, and where to write the optional preview.
struct Args {
    output: PathBuf,
...
fn parse_args() -> Args {
    ...
    while let Some(arg) = args.next() {
        if arg == "--preview" {
            preview = Some(PathBuf::from(args.next().expect("--preview takes a path")));
        } else {
