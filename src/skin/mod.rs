//! Ported from fastpotify (MIT, Copyright (c) 2026 Carmine Paolino), src/skin/mod.rs.
//!
...
//! [`layout`] defines window positions. Missing files fall back to the
//! built-in classic skin of ytamp. Modern `.wal` skins are unsupported.
...
impl fmt::Display for SkinError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            SkinError::Io(error) => write!(f, "{error}"),
            SkinError::NotAnArchive => write!(
                f,
                "not a zip archive; a classic skin is a .wsz file or a folder of bitmaps"
            ),
            SkinError::Archive(error) => write!(f, "{error}"),
            SkinError::ModernSkin => write!(
                f,
                "this is a modern Winamp skin, which ytamp cannot draw; it needs a classic one"
            ),
            SkinError::Empty => write!(f, "no skin bitmaps were found inside"),
        }
    }
}
...
impl Skin {
    ...
    /// The skin ytamp ships, so a run has a look before the listener picks
    /// another, and so any gaps in another skin can be filled from it. It
    /// is packed as a `.wsz` like any other, so it goes through the same
    /// reader.
...
/// The built-in skin, ported from `examples/default_skin.rs` of fastpotify.
