//! Ported from fastpotify (MIT, Copyright (c) 2026 Carmine Paolino), src/skin/zip.rs.
//!
...
impl fmt::Display for ZipError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            ZipError::Truncated => write!(f, "the archive is truncated"),
            ZipError::Unsupported(name) => {
                write!(
                    f,
                    "{name} uses a compression method this reader does not support"
                )
            }
            ZipError::Encrypted(name) => write!(f, "{name} is encrypted"),
            ZipError::TooLarge(name) => write!(f, "{name} is too large to be part of a skin"),
            ZipError::Inflate(name, error) => write!(f, "{name} could not be inflated: {error}"),
        }
    }
}
...
impl<'a> Archive<'a> {
    pub fn parse(data: &'a [u8]) -> Result<Self, ZipError> {
        ...
        for _ in 0..count {
            let (entry, record_len) = read_central_entry(data, cursor)?;
            entries.push(entry);
            cursor += record_len;
        }
    ...
    pub fn read(&self, entry: &Entry) -> Result<Vec<u8>, ZipError> {
        check_readable(entry)?;
        let raw = local_payload(self.data, entry)?;
        inflate(entry, raw)
    }
}

/// Rejects an entry `read` cannot safely inflate: encrypted, or too big to
/// be a skin bitmap.
fn check_readable(entry: &Entry) -> Result<(), ZipError> {
...
/// The compressed bytes of an entry, found through its local file header.
fn local_payload<'a>(data: &'a [u8], entry: &Entry) -> Result<&'a [u8], ZipError> {
    ...
    slice(data, at + 30 + name_len + extra_len, entry.compressed_size)
}

/// Decompresses `raw` per the entry's method: copied as is, or inflated.
fn inflate(entry: &Entry, raw: &[u8]) -> Result<Vec<u8>, ZipError> {
    match entry.method {
...
/// A central directory record's fixed part: 46 bytes, read as one slice so
/// the fields inside it need no further bounds checks.
const CENTRAL_RECORD_LEN: usize = 46;

/// One record of the central directory, starting at `cursor`: the entry it
/// describes, and the record's length so the caller can step past it.
fn read_central_entry(data: &[u8], cursor: usize) -> Result<(Entry, usize), ZipError> {
    let header = slice(data, cursor, CENTRAL_RECORD_LEN)?;
    if read_u32(header, 0) != CENTRAL_HEADER {
        return Err(ZipError::NotAnArchive);
    }
    ...
    let name_bytes = slice(data, cursor + CENTRAL_RECORD_LEN, name_len)?;
    let entry = Entry {
        name: String::from_utf8_lossy(name_bytes).into_owned(),
        method: read_u16(header, 10),
        flags: read_u16(header, 8),
        compressed_size: read_u32(header, 20) as usize,
        size: read_u32(header, 24) as usize,
        header_offset: read_u32(header, 42) as usize,
    };
    Ok((
        entry,
        CENTRAL_RECORD_LEN + name_len + extra_len + comment_len,
    ))
}

/// Reads a little-endian `u16` from an in-bounds offset of a slice already
/// checked to be long enough, so this cannot fail.
fn read_u16(bytes: &[u8], at: usize) -> u16 {
    u16::from_le_bytes(bytes[at..at + 2].try_into().expect("in bounds"))
}

/// Reads a little-endian `u32` from an in-bounds offset of a slice already
/// checked to be long enough, so this cannot fail.
fn read_u32(bytes: &[u8], at: usize) -> u32 {
    u32::from_le_bytes(bytes[at..at + 4].try_into().expect("in bounds"))
}
