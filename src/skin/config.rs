//! Ported from fastpotify (MIT, Copyright (c) 2026 Carmine Paolino), src/skin/config.rs.
//!
...
impl Mask {
    ...
    pub fn from_polygons(width: u32, height: u32, polygons: &[Vec<(i32, i32)>]) -> Self {
        let rows = (0..height)
            .map(|y| row_spans(width, polygons, y as f64 + 0.5))
            .collect();
        Self {
            width,
            height,
...
/// The visible spans of one row, at `centre_y`, filled the even-odd way
/// from every polygon with at least three points.
fn row_spans(width: u32, polygons: &[Vec<(i32, i32)>], centre_y: f64) -> Vec<(u32, u32)> {
    let mut inside = vec![false; width as usize];
    for polygon in polygons.iter().filter(|polygon| polygon.len() >= 3) {
        fill_polygon_row(&mut inside, polygon, centre_y);
    }
    spans_of(&inside)
}

/// Marks the pixels of one row inside a single polygon's even-odd fill.
fn fill_polygon_row(inside: &mut [bool], polygon: &[(i32, i32)], centre_y: f64) {
    let width = inside.len();
    let mut crossings = row_crossings(polygon, centre_y);
    crossings.sort_by(f64::total_cmp);
    for pair in crossings.as_chunks::<2>().0 {
        let start = ((pair[0] - 0.5).ceil().max(0.0)) as usize;
        let end = ((pair[1] - 0.5).ceil().max(0.0) as usize).min(width);
...
/// Where a polygon's edges cross the horizontal line at `centre_y`, as x
/// coordinates, unsorted.
fn row_crossings(polygon: &[(i32, i32)], centre_y: f64) -> Vec<f64> {
...
/// The runs of `true` in a row of pixels, as `start..end` spans.
fn spans_of(inside: &[bool]) -> Vec<(u32, u32)> {
    ...
    if let Some(from) = start {
        spans.push((from, inside.len() as u32));
    }
    spans
}
...
pub fn parse_regions(text: &str) -> Regions {
    ...
    for line in text.lines() {
        ...
        if let Some(name) = line.strip_prefix('[') {
            commit_section(&mut regions, &section, &num_points, &points);
            section = name.trim_end_matches(']').trim().to_ascii_lowercase();
            num_points.clear();
            points.clear();
            continue;
        }
        record_field(line, &mut num_points, &mut points);
    }
    commit_section(&mut regions, &section, &num_points, &points);
    regions
}

/// Reads one `key=value` line of `region.txt` into the section's running
/// point lists. A line that is not a recognised key changes nothing.
fn record_field(line: &str, num_points: &mut Vec<usize>, points: &mut Vec<i32>) {
...
/// A window's size, paired with a way to reach the field of `Regions` its
/// mask belongs in.
type RegionSlot = (u32, u32, fn(&mut Regions) -> &mut Option<Mask>);

/// A section's window size and the field of `Regions` it fills, by name.
fn region_slot(section: &str) -> Option<RegionSlot> {
    match section {
        "normal" => Some((275, 116, |regions: &mut Regions| &mut regions.normal)),
        "windowshade" => Some((275, 14, |regions: &mut Regions| &mut regions.shade)),
        "equalizer" => Some((275, 116, |regions: &mut Regions| &mut regions.equalizer)),
        "equalizerws" => Some((275, 14, |regions: &mut Regions| {
            &mut regions.equalizer_shade
        })),
        _ => None,
    }
}

/// Turns one section's point lists into a mask and stores it, unless the
/// section named nothing this reader knows, has no points, or covers the
/// whole window and so needs no mask at all.
fn commit_section(regions: &mut Regions, section: &str, num_points: &[usize], points: &[i32]) {
    if num_points.is_empty() || points.is_empty() {
        return;
    }
    let Some((width, height, slot)) = region_slot(section) else {
        return;
    };
    let polygons = assemble_polygons(num_points, points);
    let mask = Mask::from_polygons(width, height, &polygons);
    if !mask.is_everything() {
        *slot(regions) = Some(mask);
    }
}

/// Splits a flat `x,y` point list into polygons of the sizes `num_points`
/// gives, dropping any polygon short of the points it claimed or too
/// small to enclose anything.
fn assemble_polygons(num_points: &[usize], points: &[i32]) -> Vec<Vec<(i32, i32)>> {
