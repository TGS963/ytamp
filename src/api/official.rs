...
/// A liked list can hold thousands of entries (one user: 4875), and
/// every page costs a request plus a duration lookup. The newest ten
/// pages, 500 tracks, load; the full list waits for a load-more UI.
const LIKED_PAGE_CAP: usize = 10;
...
impl DataApi {
    ...
    pub async fn liked_songs(&self) -> Result<Vec<Track>, String> {
        self.tracks_of(LIKED_VIDEOS_PLAYLIST, LIKED_PAGE_CAP).await
    }

    pub async fn playlist_tracks(&self, id: &PlaylistId) -> Result<Vec<Track>, String> {
        self.tracks_of(&id.0, NO_PAGE_CAP).await
    }

    async fn tracks_of(&self, playlist_id: &str, page_cap: usize) -> Result<Vec<Track>, String> {
        let url =
            format!("{BASE}/playlistItems?part=snippet&playlistId={playlist_id}&maxResults=50");
        let pages = self.get_pages(&url, page_cap).await?;
        let mut tracks: Vec<Track> = pages
            .iter()
            .flat_map(items_of)
    ...
    async fn get_pages(&self, url: &str, page_cap: usize) -> Result<Vec<Value>, String> {
        ...
        loop {
            let page_url = match page_token.is_empty() {
                true => url.to_string(),
                false => format!("{url}&pageToken={page_token}"),
            };
